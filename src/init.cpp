...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all GROW specific functionality (Masternodes) (0-1, default: %u)"), 0));
...
/** Sanity checks
 *  Ensure that MrGrower is running in a usable environment with all
...
#endif
    LogPrintf("MrGrower version %s (%s)\n", version_string, CLIENT_DATE);
}
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. MrGrower is shutting down."));

    std::string strDataDir = GetDataDir().string();

    // Make sure only a single MrGrower process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. MrGrower is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                // MrGrower: load previous sessions sporks if we have them.
