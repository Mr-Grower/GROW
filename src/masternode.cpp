...
{
    if (nHeight > 7500000)       return 20000 * COIN;
    else if (nHeight > 5000000)  return 17500 * COIN;
    else if (nHeight > 3000000)  return 15000 * COIN;
    else if (nHeight > 10000)  return 10000 * COIN;
...
CAmount CMasternode::GetMasternodePayment(int nHeight)
{
    ...
    if (nHeight > 10000) return blockValue * 80 / 100;
    return 0;
}
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 GROW tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                   // block for 1000 GROW tx -> 1 confirmation
