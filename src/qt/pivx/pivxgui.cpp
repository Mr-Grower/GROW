...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("MrGrower client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("MrGrower"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("MrGrower"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
