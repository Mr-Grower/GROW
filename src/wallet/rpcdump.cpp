...
UniValue importprivkey(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
        throw std::runtime_error(
            "importprivkey \"growerprivkey\" ( \"label\" rescan fStakingAddress )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"growerprivkey\"      (string, required) The private key (see dumpprivkey)\n"
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "dumpprivkey \"groweraddress\"\n"
            "\nReveals the private key corresponding to 'groweraddress'.\n"
            "Then the importprivkey can be used with this output\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"groweraddress\"   (string, required) The grower address for the private key\n"
    ...
    if (!IsValidDestination(dest))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid MrGrower address");
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by MrGrower %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
    ...
    if (!seed_id.IsNull())
    {
        ...
        if (pwalletMain->GetKey(seed_id, seed)) {
            ...
            CExtKey cointypeKey;           //key at m/purpose'/coin_type'  --> key at m/44'/846'
            CExtKey accountKey;            //key at m/purpose'/coin_type'/account' ---> key at m/44'/846'/0'
            CExtKey changeKey;             //key at m/purpose'/coin_type'/account'/change ---> key at m/44'/846'/0'/HDChain::ChangeType::ECOMMERCE.
...
UniValue bip38encrypt(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "bip38encrypt \"groweraddress\" \"passphrase\"\n"
            "\nEncrypts a private key corresponding to 'groweraddress'.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"groweraddress\"   (string, required) The grower address for the private key (you must hold the key already)\n"
...
UniValue bip38decrypt(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "bip38decrypt \"groweraddress\" \"passphrase\"\n"
